...
## Exact Next Step
> **2026-06-20 (sera) — IMPORT BLOCCATO: sessione AutoDS scaduta.** Tentato import dei 5 prodotti ricercati (GO owner): la pagina `/upload` mostra **Drafts (0)** (erano 25-28) e la UI "Add product with link" non c'è → quadro di **sessione logged-out/scaduta** (NON dati persi: 9 listing live su eBay restano, le draft esistono lato-account). **Sblocco:** rifare login AutoDS → rigenerare `storage_state.json` (`login_and_save_session.py`, GO-class + serve owner per credenziali/eventuale CAPTCHA). I 5 prodotti sono pronti (URL+titolo+descrizione+comandi committati `e9a333d`): import ripartirà appena la sessione è valida. Vedi `…/listings/LISTING_RESEARCH_10_2026-06-20.md`.
>
