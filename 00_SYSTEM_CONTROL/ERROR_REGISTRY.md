...
- **TEST DI REGRESSIONE:** `set_location_china_kbd.py <id> <guard> 0` → stampa `VERIFY item_country_location = <valore>`: è PASS solo se ritorna "China"; finché ritorna "United States" la via è murata. Evidenza: questa sessione (selector="China" ma API="United States"). [Riprovare solo via UI umana / browser visibile.]
